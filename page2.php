...
if($step == 1){
    ...
    $query1["vsbState"] = 'Shipped';
    $query2["vsbState"] = 'Shipped';
...
// comment the old one and use this new edition lyq.
    $resultArray = array();
    foreach($result1 as $arr1){
       foreach($result2 as $arr2){
           if($arr1['bsp'] == $arr2['bsp'] && $arr1['board'] == $arr2['board'] && $arr1['mp'] == $arr2['mp'] && $arr1['bits'] == $arr2['bits'])
           {
    ...
    return array(
        ...
        array(
            '$group' => array(
                '_id' => array(
                    'bsp' => '$bsp',
// that is because 0510 document have the boardtype content ,but before 0510 ,
//all the boardtype is null .if we don't comment this line ,the compare will have no results.
                    //'board' => '$board',
                    'mp' => '$mp',
                    'bits' => '$bits'
                )
